...
{
    protected $url = 'http://localhost:9200/product/product';
	...
	//$data['query']['bool']['should']['exists']['field'] = 'cover_image'; // cover_image != null
	$data['query']['bool']['filter']['bool']['should']['exists']['field'] = 'cover_image'; // cover_image != null
	...
	//echo $data;die;
	//Log::info('params:'.json_encode($data)); //debug
    ...
    {
        $data['query']['bool']['must'][]['term']['status'] = 1;
	...
	if (isset($condition['rank'])) {
            $data['query']['bool']['filter']['range']['rank']['lt'] = $condition['rank'];
	}
	if (isset($condition['discount'])) {
	    if (isset($data['query']['bool']['filter']['range'])) {
		$data['query']['bool']['should'][]['term']['discount_id'] = 7;
		$data['query']['bool']['should'][]['term']['discount_id'] = 8;
	    } else {
                $data['query']['bool']['filter']['range']['discount']['lte'] = $condition['discount'];
	    }
	}
	if (isset($condition['store'])) {
            $data['query']['bool']['must'][]['term']['store'] = $condition['store'];
	}
        $data['script']['inline'] = 'ctx._source.status += '.$update_data['status'];
	//Log::info('Update by Query: '.var_export($data, true)); //debug
    ...
    private function updateByRank($condition, $update_data)
    {
        $data['query']['bool']['filter']['range']['rank']['gte'] = $condition['rank'];
        $data['script']['inline'] = 'ctx._source.status += '.$update_data['status'];
        $url = $this->url."/_update_by_query?conflicts=proceed";
        $response = Curl::to($url)
            ->withData($data)
	...
	if (isset($condition['category'])) {
        ...
        if (isset($condition['discount'])) {
            $data['query']['bool']['filter']['range']['discount']['lt'] = $condition['discount'];
        }
        if (isset($condition['store'])) {
            $data['query']['bool']['must'][]['term']['store'] = $condition['store'];
        }
